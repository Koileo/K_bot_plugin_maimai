...
class DrawBest(ScoreBaseImage):
    def __init__(self, UserInfo: UserInfo_all, qqId: Optional[Union[int, str]] = None) -> None:
        super().__init__(Image.open(maimaidir / 'b100_bg.png').convert('RGBA'))
        ...
        if len(sorted_sdlist) >= 100:
            self.sdBest = sorted_sdlist[:100]
        else:
...
async def generate_allb100(qqid: Optional[int] = None, username: Optional[str] = None) -> str:
    try:
        if username:
            qqid = None
