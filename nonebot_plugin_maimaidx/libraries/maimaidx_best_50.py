...
from .maimaidx_model import ChartInfo, PlayInfoDefault, PlayInfoDev, UserInfo,UserInfo_all
...
async def backname(qqid: Optional[int] = None, username: Optional[str] = None) -> str:
    try:
        if username:
            qqid = None
        ...
        return mai_info.nickname  # 确保成功时返回正确的昵称
    except UserNotFoundError as e:
        msg = str(e)
        log.error(msg)
        return "用户未找到"  # 返回一个默认值
    except UserDisabledQueryError as e:
        msg = str(e)
        log.error(msg)
        return "查询被禁用"  # 返回一个默认值
    except Exception as e:
        log.error(traceback.format_exc())
        return "未知错误，无法获取昵称"  # 返回一个默认值


async def backrating(qqid: Optional[int] = None, username: Optional[str] = None) -> str:
    try:
        if username:
            qqid = None
        userinfo = await maiApi.query_user_dev(qqid=qqid, username=username)
        ...
        mai_info = UserInfo_all(**userinfo)
        return mai_info.rating  # 确保成功时返回正确的评分
    except UserNotFoundError as e:
        msg = str(e)
        log.error(msg)
        return "用户未找到"  # 返回一个默认值
