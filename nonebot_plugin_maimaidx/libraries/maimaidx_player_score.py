...
class DrawScore(ScoreBaseImage):
    ...
    def whilerisepic(self, data: List[RiseScore], low_score: int, isdx: bool):
        ...
        for index, _d in enumerate(data):
            ...
            self._im.alpha_composite(Image.open(maimaidir / f'{_d.type.upper()}.png').resize((60, 22)),
                                     (x + 240, y + 114))
            if _d.oldrate:
    ...
    def draw_rise(self, sd: List[RiseScore], sd_score: int, dx: List[RiseScore], dx_score: int) -> Image.Image:
        ...
        self._sy.draw(700, height - 76, 18,
                      f'Designed by Yuri-YuzuChaN & BlueDeer233. Generated by {maiconfig.botName} BOT', self.text_color,
                      'mm')
        return self._im
    ...
    def draw_plan(
        ...
        self._sy.draw(700, 77 + completed_y + unfinished_y, 22, f'未游玩谱面「{len(notstarted)}」个', self.text_color,
                      'mm')
    ...
    def draw_scorelist(
        ...
        for n in range(r):
            ...
            self._sy.draw(700, 77 + y, 28, f'No.{start}- No.{start + len(newdata[n * 20: (n + 1) * 20]) - 1}',
                          self.text_color, 'mm')
...
async def level_achievement_list_data(
    ...
    try:
        ...
        if isinstance(rating, str):
            newdata = sorted(list(filter(lambda x: x.level == rating, data)), key=lambda z: z.achievements,
                             reverse=True)
        else:
...
async def level_table_achievement_list_data(
        qqid: int,
        username: Optional[str],
        rating: Union[str, float],
        page: int = 1
    ...
    try:
        ...
        obj = await maiApi.query_user_get_dev(qqid=qqid, username=username)
        data = obj.records
        newdata = sorted(list(filter(lambda x: x.level_index == rating, data)), key=lambda z: z.achievements,
                             reverse=True)
