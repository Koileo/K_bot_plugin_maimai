...
class DrawBest(ScoreBaseImage):
    def __init__(self, UserInfo: UserInfo_all, qqId: Optional[Union[int, str]] = None) -> None:
        ...
        for i in self.records:
            sumdxscore = sum(mai.total_list.by_id(str(i.song_id)).charts[i.level_index].notes) * 3
            starnum = dxScore(i.dxScore / sumdxscore * 100)
            if starnum == 5 :
                isnew= song_type_map.get(str(i.song_id))
                if isnew:
                    dxlist.append(i)  # 添加到 dxlist
                else:
                    sdlist.append(i)  # 添加到 sdlist
...
async def generate_star5(qqid: Optional[int] = None, username: Optional[str] = None) -> str:
    try:
        if username:
            qqid = None
