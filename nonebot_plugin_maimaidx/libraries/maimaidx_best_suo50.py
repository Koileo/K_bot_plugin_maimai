...
class DrawBest(ScoreBaseImage):
    def __init__(self, UserInfo: UserInfo_all, qqId: Optional[Union[int, str]] = None) -> None:
        ...
        for i in self.records:
            if (i.achievements>=100.0 and i.achievements<=100.05) or (i.achievements>=100.5 and i.achievements<=100.55):
...
async def generate_suo(qqid: Optional[int] = None, username: Optional[str] = None) -> str:
    try:
        if username:
            qqid = None
