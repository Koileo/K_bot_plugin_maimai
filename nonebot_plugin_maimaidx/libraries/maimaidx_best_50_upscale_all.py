...
class DrawBest(ScoreBaseImage):
    def __init__(self, UserInfo: UserInfo_all, qqId: Optional[Union[int, str]] = None) -> None:
        ...
        for i in self.records:
            ...
            if i.achievements < 100.0:
                i.achievements = 100.00
                i.rate = 'sss'
                i.ra = int(i.ds * i.achievements * numscore_rank_map[i.rate])
            if 100.0 < i.achievements < 100.5:
                i.achievements = 100.50
                i.rate = 'sssp'
                i.ra = int(i.ds * i.achievements * numscore_rank_map[i.rate])
            if 100.5 < i.achievements :
                i.achievements = 101.00
                i.fc = 'app'
                i.rate = 'sssp'
                i.ra = int(i.ds * i.achievements * numscore_rank_map[i.rate])
            if isnew:
                dxlist.append(i)  # 添加到 apdxlist
            else:
                sdlist.append(i)  # 添加到 apsdlist
        sorted_dxlist = sorted(dxlist, key=lambda x: x.ra, reverse=True)
        sorted_sdlist = sorted(sdlist, key=lambda x: x.ra, reverse=True)
        if len(sorted_dxlist) >= 15:
            self.dxBest = sorted_dxlist[:15]
        else:
            self.dxBest = sorted_dxlist
        if len(sorted_sdlist) >= 35:
            self.sdBest = sorted_sdlist[:35]
        else:
            self.sdBest = sorted_sdlist
...
async def generate_upscaleall(qqid: Optional[int] = None, username: Optional[str] = None) -> str:
    try:
        if username:
            qqid = None
