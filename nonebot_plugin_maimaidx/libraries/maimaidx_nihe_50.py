...
numscore_rank_map = {"sssp": 0.224, "sss": 0.216, "ssp": 0.211, "ss": 0.208	, "sp": 0.203,"s": 0.2, "aaa": 0.168, "aa": 0.152, "a": 0.136, "bbb": 0.112, "bb": 0.102, "b": 0.096, "c": 0.079, "d": 0.063}
...
class DrawBest(ScoreBaseImage):
    def __init__(self, UserInfo: UserInfo_all, qqId: Optional[Union[int, str]] = None) -> None:
        ...
        self.qqid = qqId  # 统一使用 qqId
        apdxlist = []
        apsdlist = []


        with open(gamedir/'diving_music_list.json', "r", encoding="utf-8") as file1:
            song_data = json.load(file1)
        with open(chart_file, "r", encoding="utf-8") as file2:
            nihe_data = json.load(file2)
        nihe_data = nihe_data["charts"]
        song_type_map = {song["id"]: song["basic_info"].get("is_new") for song in song_data}
        song_genre_map =  {song["id"]: song["basic_info"].get("genre") for song in song_data}
        for i in self.records:
            isnew= song_type_map.get(str(i.song_id))
            try:
                if "fit_diff" in nihe_data[str(i.song_id)][i.level_index]:
                    i.ds = round(nihe_data[str(i.song_id)][i.level_index]["fit_diff"],2)
                    i.ra = int(i.ds * i.achievements * numscore_rank_map[i.rate])
            except KeyError:
                i.ds = i.ds
            if isnew:
                apdxlist.append(i)  # 添加到 apdxlist
            else:
                apsdlist.append(i)  # 添加到 apsdlist
        sorted_apdxlist = sorted(apdxlist, key=lambda x: x.ra, reverse=True)
        sorted_apsdlist = sorted(apsdlist, key=lambda x: x.ra, reverse=True)
        if len(sorted_apdxlist) >= 15:
            self.dxBest = sorted_apdxlist[:15]
        else:
            self.dxBest = sorted_apdxlist
        if len(sorted_apsdlist) >= 35:
            self.sdBest = sorted_apsdlist[:35]
        else:
            self.sdBest = sorted_apsdlist
...
async def generate_nihe(qqid: Optional[int] = None, username: Optional[str] = None) -> str:
    try:
        if username:
            qqid = None
