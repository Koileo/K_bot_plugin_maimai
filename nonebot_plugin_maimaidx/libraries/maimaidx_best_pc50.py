...
from .maimaidx_model import Record, PlayInfoDefault, PlayInfoDev, UserInfo_all
...
class ScoreBaseImage:
    ...
    def whiledraw(
            self,
            data: Union[List[Record], List[PlayInfoDefault], List[PlayInfoDev]],
            dx: bool,
            height: int = 0,
            playCounts: Optional[List[int]] = None
    ) -> None:
        """
        循环绘制成绩
        ...
        # y为第一排纵向坐标，dy为各行间距
        dy = 114
        if data and isinstance(data[0], Record):
        ...
        for num, info in enumerate(data):
            ...
            count = playCounts[num] if playCounts else 0
            self._tb.draw(x + 93, y + 65, 15, f'pc: {count}', self.t_color[info.level_index], anchor='lm')


class DrawBest(ScoreBaseImage):
    def __init__(self, UserInfo: UserInfo_all, qqId: Optional[Union[int, str]] = None) -> None:
        super().__init__(Image.open(maimaidir / 'b50_bg.png').convert('RGBA'))
        from ..command.mai_score import get_user_music, get_userid
        ...
        with open(gamedir/'diving_music_list.json', "r", encoding="utf-8") as file:
            song_data = json.load(file)
        song_type_map = {song["id"]: song["basic_info"].get("is_new") for song in song_data}
        user_id = get_userid(self.qqid)
        ...
        for i in range(1,len(old_data)):
            for j in range(0,len(old_data)-i):
                if old_data[j]['playCount'] < old_data[j+1]['playCount']:
                    old_data[j] ,old_data[j+1] = old_data[j+1], old_data[j]
        old_data.sort(key=lambda x: (-x['playCount'], -x.get('ra', 0)))
        final_result = old_data
        song_data_map = {
            (item["musicId"], item["level"]): item
        ...
        for record in self.records:
            ...
            if key in song_data_map:
                song_data = song_data_map[key]
                # 把record和对应的playCount打包存储
                pair = (record, song_data["playCount"])

                is_new = song_type_map.get(str(record.song_id))
                if is_new:
                    dx_pairs.append(pair)
                else:
                    sd_pairs.append(pair)

        dx_pairs.sort(key=lambda x: (-x[1], -x[0].ra))
        sd_pairs.sort(key=lambda x: (-x[1], -x[0].ra))

        dxlist = [pair[0] for pair in dx_pairs[:15]]
        sdlist = [pair[0] for pair in sd_pairs[:35]]

        dx_playCounts = [pair[1] for pair in dx_pairs[:15]]
        sd_playCounts = [pair[1] for pair in sd_pairs[:35]]

        self.sdBest = sdlist[:35]
        self.dxBest = dxlist[:15]
        self.sd_playCounts = sd_playCounts
        self.dx_playCounts = dx_playCounts

    def _findRaPic(self) -> str:
        sdrating = sum([computeRa(acc.ds, acc.achievements, israte=True)[0] for acc in self.sdBest])
        dxrating = sum([computeRa(acc.ds, acc.achievements, israte=True)[0] for acc in self.dxBest])
        self.Rating = sdrating + dxrating
        if self.Rating < 1000:
            num = '01'
    ...
    async def draw(self) -> Image.Image:
        ...
        self.whiledraw(self.sdBest, False, playCounts=self.sd_playCounts)
        self.whiledraw(self.dxBest, True, playCounts=self.dx_playCounts)
...
async def generate_pcb50(qqid: Optional[int] = None, username: Optional[str] = None) -> str:
    try:
        if username:
            qqid = None
        userinfo = await maiApi.query_user_dev(qqid=qqid, username=username)
        mai_info = UserInfo_all(**userinfo)
