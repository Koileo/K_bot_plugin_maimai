...
class DrawBest(ScoreBaseImage):
    def __init__(self, UserInfo: UserInfo_all, qqId: Optional[Union[int, str]] = None) -> None:
        ...
        for i in self.records:
            if i.fc == "app" or i.fc == "ap" or i.fc == "fcp" or i.fc == "fc":
                isnew= song_type_map.get(str(i.song_id))
                if isnew:
                    fcdxlist.append(i)  # 添加到 apdxlist
                else:
                    fcsdlist.append(i)  # 添加到 apsdlist
        sorted_fcdxlist = sorted(fcdxlist, key=lambda x: x.ra, reverse=True)
        sorted_fcsdlist = sorted(fcsdlist, key=lambda x: x.ra, reverse=True)
        if len(sorted_fcdxlist) >= 15:
            self.dxBest = sorted_fcdxlist[:15]
        else:
            self.dxBest = sorted_fcdxlist
        if len(sorted_fcsdlist) >= 35:
            self.sdBest = sorted_fcsdlist[:35]
        else:
            self.sdBest = sorted_fcsdlist
...
async def generate_fc(qqid: Optional[int] = None, username: Optional[str] = None) -> str:
    try:
        if username:
            qqid = None
