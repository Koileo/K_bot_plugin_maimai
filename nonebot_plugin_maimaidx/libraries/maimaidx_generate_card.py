...
maimaiImgPath = os.path.join(os.getcwd(), "res", "images")
materialPath = os.path.join(os.getcwd(), "res", "material")
...
async def circle_corner(img, radii=30):
    # 白色区域透明可见，黑色区域不可见
...
async def drawUserImg(title,totalRating,rankRating,userName,icon,plate,title_rare="Normal",classRank=-1):
    ...
    ratingPlateImg = Image.open(rf"{maimaiImgPath}/Rating/{await getRatingBgImg(totalRating)}").resize((174,36))
    ...
    UserIdImg = await circle_corner(Image.new('RGBA', (270, 40), color=(255, 255, 255)),5)
    ...
    await draw_text_with_stroke_and_spacing(
        totalRatingDraw,
...
async def getRatingBgImg(rating):
...
async def apply_gradient_blur(image, blur_radius=10, start_height=0, end_height=35):
...
async def drawCharaImgNewSub(charaId, charaLevel):
    if int(charaLevel) > 9999:
        charaLevel = 9999
    if int(charaLevel) < 0:
        charaLevel = 0
    star, progress = await get_star_info(int(charaLevel))
...
async def drawCharaImgNewMain(charaId, charaLevel):
    if int(charaLevel) > 9999:
        charaLevel = 9999
    if int(charaLevel) < 0:
        charaLevel = 0
    ...
    return await apply_gradient_blur(base.resize((264, 264)), 100)

async def get_dominant_color(image):
    # 颜色模式转换，以便输出rgb颜色值
...
async def draw_text_with_stroke(draw: ImageDraw, pos, text, font, fill_color, stroke_width=2, stroke_color='white'):
    # 绘制描边
...
async def draw_text_with_stroke_and_spacing(draw: ImageDraw.ImageDraw, pos, text, font, fill_color, stroke_width=2, stroke_color='white', spacing=5):
    # 绘制描边
    for x_offset in range(-stroke_width+1, stroke_width):
        for y_offset in range(-stroke_width+1, stroke_width):
            if x_offset == 0 and y_offset == 0:
            ...
            await draw_text_with_spacing(draw, (xx, yy), text, font, stroke_color, spacing)

    await draw_text_with_spacing(draw, pos, text, font, fill_color, spacing)

async def draw_text_with_spacing(draw: ImageDraw.ImageDraw, pos, text, font, fill_color, spacing=5):
    # 逐字符绘制并调整位置
    x, y = pos
    for char in text:
...
async def call_user_img(user_data, no_chara=False):
    try:
    ...
    theme_color = await get_dominant_color(frame_img)
    ...
    UserImg = await drawUserImg(user_data["title"], user_data["rating"], user_data['courseRank'], user_data['nickname'],
                                icon, plate, user_data["titleRare"], user_data["classRank"])
    ...
    if not no_chara:
        main_chara_img = await drawCharaImgNewMain(user_data["chara"][0], user_data["charaLevel"][0])
        main_chara = main_chara_img.resize((309, 309))  # 先获取图片对象再调整大小
        img.paste(main_chara, (-36, 143), main_chara)

        chara_start_x = 204
        chara_start_y = 170
        for chara in zip(user_data["chara"][1:], user_data["charaLevel"][1:]):
            chara_img = await drawCharaImgNewSub(*chara)
            chara_img_resized = chara_img.resize((147, 290))  # 先获取图片对象再调整大小
            img.paste(chara_img_resized, (chara_start_x, chara_start_y), chara_img_resized)
            chara_start_x += 138

    designDraw = ImageDraw.Draw(img)
    designDraw.text((20, 457),
                    f"Powered by Cachebot| Designed by LuoYe_XwX  图片仅供参考",
                    font=ImageFont.truetype(rf'{materialPath}/GenSenMaruGothicTW-Bold.ttf', 12), fill=text_color)

    return img

async def call_user_img_preview(user_data):
    ...
    ratingPlateImg = Image.open(rf"{maimaiImgPath}/Rating_big/{await getRatingBgImg(int(user_data['rating']))}").resize((312,58))
...
async def generate_image_maicard(user_data):
    img = await call_user_img(user_data, False)
