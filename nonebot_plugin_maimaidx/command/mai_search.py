...
async def _(event: MessageEvent, end: str = Endswith()):
    ...
    if name.isdigit() and (music := mai.total_list.by_id(name)):
        await search_alias_song.finish('您要找的是不是：' + await draw_music_info(music, event.user_id),
                                       reply_message=True)
    ...
    if len(result) == 0:
        ...
        await search_alias_song.finish('您要找的是不是：' + await draw_music_info(result.random(), event.user_id),
                                       reply_message=True)
    elif len(result) < 50:
