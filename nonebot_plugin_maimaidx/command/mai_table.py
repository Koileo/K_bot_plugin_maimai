...
level_table_achievement_list = on_regex(r'^(紫|红|绿|白|黄)\s?分数列表\s?([0-9]+)?\s?(.+)?')
...
@level_table_achievement_list.handle()
async def _(event: MessageEvent, match=RegexMatched(), user_id: Optional[int] = Depends(get_at_qq)):
    qqid = user_id or event.user_id

    level_table = match.group(1)
    page = match.group(2)
    username = match.group(3)
    ...
    try:
        if level_table == "紫":
            ...
            await level_achievement_list.finish('无此难度', reply_message=True)
    except ValueError:
        await level_achievement_list.finish('无此难度', reply_message=True)

    data = await level_table_achievement_list_data(qqid, username, level_index, int(page) if page else 1)
    await level_achievement_list.finish(data, reply_message=True)